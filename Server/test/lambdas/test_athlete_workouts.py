...
from lambdas.athlete.quick_input.quick_input import quick_input
...
def test_quick_input_time_distance_for_self_returns_success():
    # Arrange
    event = {
        "body": json.dumps({
            "input": {"distance": 150, "time": 20.5, "type": "run"},
            "athleteId": "1234",
            "date": date
        }),
        "headers":generate_auth_header("1234", "Athlete", "test_athlete")
    ...
    # Act
    response = quick_input(event, {})

    # Assert
    assert response['statusCode'] == 200
    inputs = fetch_all("SELECT athleteId, distance, time FROM athlete_time_inputs WHERE athleteId = '1234'")
    assert inputs is not None
    assert len(inputs) == 1
    input = inputs[0]
    assert input[0] == '1234'
...
def test_quick_input_rest_time_for_self_returns_success():
    # Arrange
    event = {
        "body": json.dumps({
            "input": {"restTime": 8, "type": "rest"},
            "athleteId": "1234",
            "date": date
        }),
        "headers":generate_auth_header("1234", "Athlete", "test_athlete")
    ...
    # Assert
    assert response['statusCode'] == 200
    inputs = fetch_all("SELECT athleteId, restTime FROM athlete_rest_inputs WHERE athleteId = '1234'")
    assert inputs is not None
    assert len(inputs) == 1
    input = inputs[0]
    assert input[0] == '1234'
...
def test_quick_input_time_distance_for_athlete_by_coach_returns_success():
    # Arrange
    event = {
        "body": json.dumps({
            "input": {"distance": 150, "time": 20.5, "type": "run"},
...
def test_quick_input_rest_time_for_athlete_by_coach_returns_success():
    # Arrange
    event = {
        "body": json.dumps({
            "input": {"restTime": 8, "type": "rest"},
...
def test_quick_input_for_athlete_by_unauthorized_user_returns_forbidden():
    # Arrange
    create_extra_athlete("test2", "1235")
    event = {
        "body": json.dumps({
        ...
        "headers":generate_auth_header("1235", "Athlete", "test2") # Different athlete
    }
