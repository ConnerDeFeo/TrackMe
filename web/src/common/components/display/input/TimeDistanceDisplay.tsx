...
const TimeDistanceDisplay = ({ time, distance, selected }: { time: number; distance: number; selected?: boolean }) => {
  return (
    <div className={`flex flex-row justify-between items-center bg-blue-100 p-2 rounded-md mb-1 border-2 ${selected ? "border-red-500" : "border-transparent"}`}>
      <p className="font-semibold">Distance: {distance}m</p>
